 ...
 * under the License.
 */
var gadgetServer = gadgetServer || {};
gadgetServer.opensocial = gadgetServer.opensocial || (function() {
    ...
    function validateAndRenderGadget(gadget) {
        ...
        if (validationResult.valid) {
            ...
            gadgetServer.errorWidget(gadget.regionWidgetId, gadgetServer.getClientMessage("opensocial.render_error") +  "<br /><br />" + validationResult.error);
        }
    }
    ...
    function renderNewGadget(gadget) {
        ...
        gadget.maximize = function() {
            ...
            renderGadgetView(gadgetServer.opensocial.VIEW_NAMES.CANVAS, this);
        };
        gadget.minimize = function() {
            renderGadgetViewIfNotCollapsed(gadgetServer.opensocial.VIEW_NAMES.HOME, this);
        };
        ...
        gadget.restore = function() {
            renderGadgetView(gadgetServer.opensocial.VIEW_NAMES.HOME, gadgetServer.getRegionWidgetById(this.regionWidgetId));
        };
        gadget.savePreferences = function(userPrefs) {
            this.userPrefs = userPrefs;
            gadgetServer.api.rest.saveWidgetPreferences({regionWidgetId: this.regionWidgetId, userPrefs: userPrefs});
            // re-render the gadget in the same view if the gadget is not collapsed
            renderGadgetViewIfNotCollapsed(gadgetServer.opensocial.getCurrentView(this.regionWidgetId), this);
        };
        gadget.editCustomPrefs = function() {
            // display the gadget in custom edit prefs view
            renderGadgetView(gadgetServer.opensocial.VIEW_NAMES.PREFERENCES, this);
        };
        ...
        if (gadget.metadata.hasPrefsToEdit || gadget.metadata.views.preferences) {
            if ( gadget.metadata.views.preferences != undefined ) {
                gadgetServer.layout.enableEditPrefsMenuItem(gadget.regionWidgetId, true);
            }
            else {
                gadgetServer.layout.enableEditPrefsMenuItem(gadget.regionWidgetId, false);
            }
        }

        // if the gadget is not collapsed, render it
        renderGadgetViewIfNotCollapsed(gadgetServer.opensocial.VIEW_NAMES.HOME, gadget);
    }
    ...
    function getSizeFromElement(id, view) {
        ...
        return {width: elem.clientWidth - OFFSET, height: view == gadgetServer.opensocial.VIEW_NAMES.CANVAS ? elem.clientHeight : MIN_HEIGHT};
    }
    ...
    function getActiveGadgetHolderByWidgetId(widgetId) {
        return gadgetServer.getRegionWidgetById(widgetId).site.getActiveGadgetHolder();
    }
    ...
    function setPref(args, editToken, prefName, prefValue) {
        var widgetId = gadgetServer.getObjectIdFromDomId(args.gs.getActiveGadgetHolder().getElement().id);
        var regionWidget = gadgetServer.getRegionWidgetById(widgetId);
        // update the memory prefs object
        regionWidget.userPrefs[prefName] = prefValue;
        // persist it to database
        gadgetServer.api.rest.saveWidgetPreference({regionWidgetId: widgetId, userPref: {prefName: prefName, prefValue: prefValue}});
    }
    ...
    function requestNavigateTo(args, viewName) {
        ...
        switch(viewName) {
            case VIEW_NAMES.CANVAS:
                gadgetServer.maximizeWidget(fnArgs);
                break;
            case VIEW_NAMES.HOME:
                gadgetServer.minimizeWidget(fnArgs);
                break;
            case VIEW_NAMES.PREFERENCES:
                gadgetServer.editCustomPrefs(fnArgs);
                break;
        }
    }
...
gadgetServer.registerProvider(gadgetServer.opensocial);