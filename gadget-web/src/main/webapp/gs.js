 ...
 * under the License.
 */
var gadgetServer = gadgetServer || (function() {
    var providerMap = {};
    var widgetByIdMap = {};
    var context = "";
    var clientMessages = {};
    ...
    var ui = (function() {
        ...
        function dragStop(event, ui) {
            ...
            gadgetServer.api.rpc.moveWidget(uiState);
            clearState();
        }
        ...
        function initMobileWidgetUI() {
            $(".widget-wrapper").each(function(){
                var widgetId = extractObjectIdFromElementId($(this).attr("id"));
                var widget = gadgetServer.getRegionWidgetById(widgetId);
        ...
        function maximizeAction(args) {
            ...
            var widget = gadgetServer.getRegionWidgetById(regionWidgetId);
        ...
        function toggleCollapseAction(args) {
            ...
            if (gadgetServer.isMobile()) {
                gadgetServer.doWidgetUiCollapse(functionArgs);
            } else {
                gadgetServer.api.rest.saveWidgetCollapsedState(functionArgs);
            }
        }

        function doWidgetUiCollapse(args) {
            // update the in-memory widget with the new collapsed status
            gadgetServer.getRegionWidgetById(args.regionWidgetId).collapsed = args.collapsed;

            // toggle the collapse/restore icon
            gadgetServer.toggleCollapseWidgetIcon(args.regionWidgetId);
        ...
        function editPrefsAction(regionWidgetId) {
            ...
            if (hasRequiredUserPrefs) {
                prefsFormMarkup.push("<tr><td colspan='2' class='widget-prefs-required-text'>" + gadgetServer.getClientMessage("widget.prefs.required.title") + "</td></tr>");
            }
            ...
            prefsFormMarkup.push(PREFS_SAVE_BUTTON_TEMPLATE.replace(ELEMENT_ID_REGEX, WIDGET_PREFS_SAVE_BUTTON(regionWidget.regionWidgetId))
                .replace(BUTTON_TEXT_REGEX, gadgetServer.getClientMessage("common.save")));
            prefsFormMarkup.push(PREFS_CANCEL_BUTTON_TEMPLATE.replace(ELEMENT_ID_REGEX, WIDGET_PREFS_CANCEL_BUTTON(regionWidget.regionWidgetId))
                .replace(BUTTON_TEXT_REGEX, gadgetServer.getClientMessage("common.cancel")));
        ...
        function styleWidgetButtons(widgetId) {
            ...
            }).click({id: widgetId}, gadgetServer.minimizeWidget);
        ...
        function displayUsersOfWidget(widgetId) {
            gadgetServer.api.rest.getUsersForWidget({widgetId: widgetId, successCallback: function(data){
                ...
                $("<div class='dialog widget-users-dialog' title='" + $("#widget-" + widgetId + "-title").text().trim() + " " + gadgetServer.getClientMessage("widget.users.added_by") + "'>" + html + "</div>").dialog({
                    modal: true,
    ...
    function initializeWidgets(widgetsByRegionIdMap) {
        ...
        if (widgets.length == 0) {
            gadgetServer.displayEmptyPageMessage();
        } else {
    ...
    function initializeWidget(widget) {
        ...
        if (typeof provider == "undefined") {
            renderErrorWidget(widget.regionWidgetId, gadgetServer.getClientMessage("widget.provider.error"));
        } else {
    ...
    function viewPage(pageId) {
        ...
        window.location.href = gadgetServer.getContext() + "page/view" + fragment;
    }

    function viewWidgetDetail(widgetId, referringPageId) {
        window.location.href = gadgetServer.getContext() + "store/widget/" + widgetId + "?referringPageId=" + referringPageId;
    }
